...
if (isset($_GET['pidx'])) {
    ...
    if (isset($responseArray['status']) && $responseArray['status'] === 'Completed') {
        // Update payment status to 'completed' for ALL orders with pending status
        $stmt = $con->prepare("UPDATE `orders` SET payment_status = 'completed' WHERE payment_status = 'pending'");
        $stmt->execute();
        $stmt->close();

        echo "<script>alert('Payment Successful! Thank you for your purchase.'); window.location.href='../public/view_products.php';</script>";
    } else {
        // Handle failed or pending payment
        echo "<script>alert('Payment Failed or Pending. Please try again.'); window.location.href='../public/view_products.php';</script>";
    }
} else {
    echo "<script>alert('Invalid Payment Request.'); window.location.href='../public/view_products.php';</script>";
}
?>
