...
// Fetch keywords searched at least 3 times
$keywordSQL = "SELECT keyword, COUNT(*) as freq 
               FROM search_log 
               GROUP BY keyword 
               HAVING freq >= 3 
               ORDER BY freq DESC 
               LIMIT 5";
$keywordResult = mysqli_query($con, $keywordSQL);

$items = [];

while ($row = mysqli_fetch_assoc($keywordResult)) {
    $searchTerm = mysqli_real_escape_string($con, $row['keyword']);

    // Search in posts
    $postSQL = "SELECT id, PostTitle AS title, PostImage AS image, PostDetails AS description, 'post' AS type 
                FROM tblposts 
    ...
    // Search in products
    $productSQL = "SELECT id, name AS title, image, price, 'product' AS type 
                   FROM products 
                   WHERE name LIKE '%$searchTerm%' 
                   LIMIT 3";
    $prodRes = mysqli_query($con, $productSQL);
...
<head>
    <meta charset="UTF-8">
    <title>Popular Searched Items</title>
    <style>
        body {
            background: #0a0a0a;
            color: #fff;
...
<div class="carousel-container">
    <?php foreach ($items as $item): 
        ...
        if ($item['type'] == 'post' && isset($item['description'])) {
            ...
            $description = mb_substr($descText, 0, 120);
        }
    ?>
        <a href="<?php echo $link; ?>" class="carousel-link">
            <div class="carousel-item">
                <img src="<?php echo $imagePath; ?>" onerror="this.src='../images/default.jpg';" alt="">
                <h4><?php echo htmlspecialchars($item['title']); ?></h4>
                <?php if ($item['type'] == 'post'): ?>
                    <p title="<?php echo htmlspecialchars($description); ?>"><?php echo htmlspecialchars($description); ?></p>
